...
const STYLE = 'H(44px) Bdrs($bdrs-control) D(b) W(100%) Fz(14px) P(12px) Bdc(t)';
  ...
  render() {
    ...
    return <div>
      ...
      <form class="Maw(300px) Mx(a) Bgc(whitesmoke) P($p-panel) Bdrs($bdrs-panel) My(30px)">
        <div>
          <input class={STYLE} placeholder="Current Password" type="password" name="currentPassword" value={currentPassword} onChange={this.onChange} required/>
        </div>
        <div class="Mt($m-control)">
          <input class={STYLE} placeholder="New Password" type="password" name="newPassword" value={newPassword} onChange={this.onChange} required/>
        </div>
